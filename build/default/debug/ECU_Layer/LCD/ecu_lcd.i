...
# 11 "ECU_Layer/LCD/../../MCAL_Layer/GPIO/../compilar.h"
...
# 11 "ECU_Layer/LCD/../../MCAL_Layer/GPIO/../compilar.h" 2
...
# 50 "ECU_Layer/LCD/../../MCAL_Layer/GPIO/hal_gpio.h"
typedef enum {
    GPIO_LOW = 0,
    GPIO_HIGH
}logic_t;
...
Std_ReturnType lcd_8bit_initialize(const chr_lcd_8bit_t *lcd)
{
    ...
    else
    {
        ret &= gpio_pin_inialization(&(lcd->lcd_rs));
        ret &= gpio_pin_inialization(&(lcd->lcd_en));
        for(l_data_pin_cpounter = 0;l_data_pin_cpounter < 8;l_data_pin_cpounter++)
        {
...
Std_ReturnType lcd_8bit_send_command(const chr_lcd_8bit_t *lcd, uint8 command)
{
    ...
    else
    {
        ...
        {
            ret &= gpio_pin_write_logic(&(lcd->lcd_data[l_pin_counter]),((command>>l_pin_counter)& ((uint8)1)));
        }
...
Std_ReturnType lcd_8bit_send_char_data(const chr_lcd_8bit_t *lcd, uint8 data)
{
    ...
    else
    {
        ...
        for(l_pin_counter = 0;l_pin_counter<8;l_pin_counter++)
        {
            ret = gpio_pin_write_logic(&(lcd->lcd_data[l_pin_counter]),((data>>l_pin_counter)& ((uint8)1)));
        }
        ret &= lcd_8bit_send_enable_signal(lcd);
    }
...
static Std_ReturnType lcd_send_4bits(const chr_lcd_4bit_t *lcd,uint8 _data_command)
{
    ...
    {
        ret &=gpio_pin_write_logic(&(lcd->lcd_data[0]),((_data_command>>0) & ((uint8)1) ));
        ret &=gpio_pin_write_logic(&(lcd->lcd_data[1]),((_data_command>>1) & ((uint8)1) ));
        ret &=gpio_pin_write_logic(&(lcd->lcd_data[2]),((_data_command>>2) & ((uint8)1) ));
        ret &=gpio_pin_write_logic(&(lcd->lcd_data[3]),((_data_command>>3) & ((uint8)1) ));
    }
    return ret;
}
